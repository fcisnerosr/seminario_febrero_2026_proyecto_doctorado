...
\begin{frame}{Recordatorio: Formulacion Matematica del ICD}
    \begin{block}{Contribucion Metodologica: Indice de Calidad de Deteccion}
        Se propone una \textbf{metrica inedita} que integra tres factores normalizados para evaluar la calidad de la solucion:
        \begin{equation}
            \text{ICD} = D \times C_{\text{norm}}(\delta) \times P_{\text{FP}}(N_{\text{FP}})
        \end{equation}
        \centering \textbf{Interpretacion:} $\text{ICD} \in [0, 1]$ \quad (Donde 1.0 = Deteccion perfecta).
    \end{block}
    ...
    \begin{columns}[t]
        ...
        \textbf{2. Confianza ($C_{\text{norm}}$)}
        \begin{itemize}
            \item Escalamiento logaritmico ($\alpha=0.1$).
            \[ \frac{\ln(1+\alpha\delta)}{\ln(1+\alpha\delta_{\text{max}})} \]
        \end{itemize}
        
        \column{0.35\textwidth}
        \textbf{3. Penalizacion ($P_{\text{FP}}$)}
        \begin{itemize}
            \item Decaimiento exp. ($\beta=0.15$).
            \[ e^{-\beta N_{\text{FP}}} \]
        \end{itemize}
...
\begin{frame}{Resultados: Escenario de Abolladura}
    \begin{figure}
        \centering
        \includegraphics[height=0.75\textheight,keepaspectratio]{figs/000_08_braces_vigas_piernas_inclinadas.png}
        \caption{ubicacion de elementos estructurales en el modelo de plataforma marina}
    \end{figure}
\end{frame}
...
\begin{frame}{Corrosion: Elementos Inclinados y Resumen}
    \centering
    \textit{(Evaluacion de la metodologia ante perdida de espesor generalizada)}
